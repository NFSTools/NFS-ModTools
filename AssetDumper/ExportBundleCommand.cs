        ...
        List<LightPack> lightPacks)
    {
        ...
        foreach (var solidObject in solidsToAdd)
        {
            ...
            var geometryBuilder = new GeometryBuilder(fbxScene).WithName(solidObject.Name)
                .WithFBXProperty("Hash", $"0x{solidObject.Hash:X8}", true);
            ...
            for (var iMaterial = 0; iMaterial < solidObject.Materials.Count; iMaterial++)
            {
                ...
                {
                    string[] possibleEffectIds;
                    if (material is MostWantedMaterial)
                        possibleEffectIds = new[]
                        {
                            "WorldShader",
                            "WorldReflectShader",
                            "WorldBoneShader",
                    ...
                    else if (effectBasedMaterial is CarbonMaterial)
                        possibleEffectIds = new[]
                        {
                            "WorldShader",
                            "WorldReflectShader",
                    ...
                    else if (material is ProStreetMaterial)
                        possibleEffectIds = new[]
                        {
                            "STANDARD",
                            "TREELEAVES",
                    ...
                    else if (material is UndercoverMaterial)
                        possibleEffectIds = new[]
                        {
                            "car",
                            "car_a",
                    ...
                    else if (material is World15Material)
                        possibleEffectIds = new[]
                        {
                            "WorldShader",
                            "WorldZBiasShader",
                        ...
                        throw new Exception($"Don't know what effect list to use for type {material.GetType()}");

                    fbxMaterialBuilder = fbxMaterialBuilder.WithFBXProperty("EffectId",
                        new Enumeration((int)effectBasedMaterial.EffectId, possibleEffectIds), true);
                }

                if (material is ISortedMaterial sortedMaterial)
                    fbxMaterialBuilder =
                        fbxMaterialBuilder.WithFBXProperty("SortKey", $"0x{sortedMaterial.SortKey:X8}", true);
        ...
        foreach (var sceneNode in scene.Nodes)
        {
            ...
            if (!Matrix4x4.Decompose(sceneNode.Transform, out var scale, out var rotation, out var translation))
            {
                ...
                mesh.AddProperty(new FBXProperty<FBXSharp.ValueTypes.Vector3>("Vector3", "",
                    "OriginalTransformRow0",
                    IElementPropertyFlags.UserDefined,
                    new FBXSharp.ValueTypes.Vector3(sceneNode.Transform.M11, sceneNode.Transform.M12,
                        sceneNode.Transform.M13)));
                mesh.AddProperty(new FBXProperty<FBXSharp.ValueTypes.Vector3>("Vector3", "",
                    "OriginalTransformRow2",
                    IElementPropertyFlags.UserDefined,
                    new FBXSharp.ValueTypes.Vector3(sceneNode.Transform.M21, sceneNode.Transform.M22,
                        sceneNode.Transform.M23)));
                mesh.AddProperty(new FBXProperty<FBXSharp.ValueTypes.Vector3>("Vector3", "",
                    "OriginalTransformRow2",
                    IElementPropertyFlags.UserDefined,
                    new FBXSharp.ValueTypes.Vector3(sceneNode.Transform.M31, sceneNode.Transform.M32,
                        sceneNode.Transform.M33)));
                mesh.AddProperty(new FBXProperty<FBXSharp.ValueTypes.Vector3>("Vector3", "",
                    "OriginalTranslation",
                    IElementPropertyFlags.UserDefined,
                    new FBXSharp.ValueTypes.Vector3(sceneNode.Transform.M41, sceneNode.Transform.M42,
                        sceneNode.Transform.M43)));
            }
